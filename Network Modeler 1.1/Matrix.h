...
struct Matrix
{
	...
	template<typename F>
	void apply(F func) { for (uint32_t i = rows * cols; i--;) data[i] = func(data[i]); }
	
	template<typename F>
	void apply(F func, const Matrix* input, const Matrix* gradient) { for (uint32_t i = rows * cols; i--;) data[i] = func(data[i], input->data[i], gradient->data[i]); }
